...
| 📋 전술 | `/tactics` | 텍스트로 설명한 상황을 AI가 전술판 애니메이션으로 만들어주는 시뮬레이터 |
...
### 전술 시뮬레이터
- `/tactics`에서 로그인한 팀원이 텍스트로 축구 상황을 설명하면(예: "오른쪽 풀백 오버래핑 크로스, 스트라이커 니어포스트 침투"), 팀 맥미니의 로컬 LLM이 핵심 인물 3~6명과 움직임 화살표로 이루어진 장면 데이터를 만들고, 전술판 위에서 재생/일시정지/단계 이동이 되는 애니메이션(`components/TacticsBoard.tsx`)으로 보여줍니다.
- "경기 기록 & AI 자동 입력"과 같은 맥미니 게이트웨이(`OLLAMA_GATEWAY_URL`/`OLLAMA_GATEWAY_SECRET`/`OLLAMA_MODEL`)를 그대로 재사용합니다 — 환경변수를 늘리지 않고, 서버(`lib/tactics.ts`)가 LLM 응답을 좌표 범위·인원수·단계 수 기준으로 정리(sanitize)한 뒤 클라이언트에 내려줍니다.
