...
- **가장 중요한 규칙**: 사용자가 설명에서 특정 인물의 위치를 구역 코드로
  명시했다면(예: "A2에 있는 스트라이커", "B3의 윙백"), 그 인물은 반드시
...
A3 공간으로 전진 침투한다. 미드필더는 원터치로 A3의 윙백에게 다시
벽패스를 내준다. 패스를 받은 윙백은 A2에 있는 스트라이커에게 올려주고,
스트라이커가 마무리한다."

{
  ...
  "hasBall": true,
  "steps": [
    { "note": "윙백이 미드필더에게 패스", "positions": {"p1":"B3","p2":"B2","p3":"A2","ball":"B3"}, "arrows": [{"from":"p1","to":"B2"}] },
    { "note": "패스 후 윙백이 A3 공간으로 전진 침투", "positions": {"p1":"A3","p2":"B2","p3":"A2","ball":"B2"}, "arrows": [{"from":"p1","to":"A3"}] },
    { "note": "미드필더가 원터치로 침투한 윙백에게 벽패스", "positions": {"p1":"A3","p2":"B2","p3":"A2","ball":"A3"}, "arrows": [{"from":"p2","to":"A3"}] },
    { "note": "윙백이 스트라이커에게 올려준다", "positions": {"p1":"A3","p2":"B2","p3":"A2","ball":"A2"}, "arrows": [{"from":"p1","to":"A2"}] },
    { "note": "스트라이커가 마무리한다", "positions": {"p1":"A3","p2":"B2","p3":"A1","ball":"A1"}, "arrows": [] }
  ]
}

이 예시처럼 스트라이커(p3)는 사용자가 지정한 A2에서 시작해 끝까지 미드필더
(p2, B2)와 다른 구역에 있고, 마지막에만 슛 동작으로 A1로 살짝 움직입니다 —
사용자가 명시한 시작 구역을 절대 다른 인물과 겹치게 바꾸지 마세요.`;
