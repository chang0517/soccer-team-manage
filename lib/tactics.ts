...
- steps 개수는 사용자가 설명한 동작 수를 그대로 따르세요 — 생략하거나
  압축하지 마세요. 사용자가 "1. 2. 3."처럼 번호를 매기거나 줄바꿈으로
  동작을 나열했다면 그 개수와 정확히 똑같은 수의 step을 만드세요(다섯
  동작을 설명했으면 step도 정확히 5개). 특별히 순서를 나누지 않고 상황만
  설명했다면 자연스럽게 전개되는 단계 수(보통 2~6개)로 나누세요. 최대
  10개까지 가능합니다.
...
예시 — 사용자가 이렇게 번호로 5개 동작을 설명하면:
"1. 우측 윙백이 가운데 미드에게 패스
2. 윙백은 패스를 주고 상대팀 적진으로 돌진
3. 미드가 윙백에게 패스
4. 윙백이 패스를 받아 스트라이커에게 연결
5. 스트라이커가 골"

steps를 정확히 5개 만들어야 합니다(각 번호가 하나의 step). 예:
{
  "title": "우측 윙백-미드 연계 후 크로스 마무리",
  "players": [
  ...
  "hasBall": true,
  "steps": [
    { "note": "우측 윙백이 중앙 미드에게 패스", "positions": {"p1":{"x":85,"y":65},"p2":{"x":55,"y":55},"p3":{"x":50,"y":20},"ball":{"x":85,"y":65}}, "arrows": [{"from":"p1","to":{"x":55,"y":55}}] },
    { "note": "패스 후 윙백이 상대 진영으로 침투", "positions": {"p1":{"x":75,"y":30},"p2":{"x":55,"y":55},"p3":{"x":50,"y":20},"ball":{"x":55,"y":55}}, "arrows": [{"from":"p1","to":{"x":75,"y":30}}] },
    { "note": "미드가 침투한 윙백에게 다시 패스", "positions": {"p1":{"x":75,"y":30},"p2":{"x":55,"y":55},"p3":{"x":50,"y":20},"ball":{"x":55,"y":55}}, "arrows": [{"from":"p2","to":{"x":75,"y":30}}] },
    { "note": "윙백이 패스를 받아 스트라이커에게 연결", "positions": {"p1":{"x":75,"y":30},"p2":{"x":55,"y":55},"p3":{"x":50,"y":20},"ball":{"x":75,"y":30}}, "arrows": [{"from":"p1","to":{"x":50,"y":20}}] },
    { "note": "스트라이커가 골을 넣는다", "positions": {"p1":{"x":75,"y":30},"p2":{"x":55,"y":55},"p3":{"x":50,"y":10},"ball":{"x":50,"y":5}}, "arrows": [] }
  ]
}

이 예시처럼, 사용자가 나열한 동작 하나하나를 절대 하나의 step으로 뭉개지 말고 각각 그대로 대응시키세요.`;
