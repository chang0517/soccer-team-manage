...
- **step의 단위는 "공이 한 번 이동하는 동작"입니다.** 패스 하나, 슛 하나가
  각각 하나의 step이 되도록 나누세요. 각 step은 원칙적으로 화살표를
...
- 패스를 받으러 미리 침투하는 선수가 있다면, "뛰어가는 동작"을 별도
  step으로 쪼개지 마세요. 공이 그 공간에 도착하는 step 안에서 그 선수도
  이미 그 구역에 도착해 있는 것으로 함께 표현하세요(패스가 오는 동안
  선수가 그 자리로 침투를 끝낸 것으로 간주). 즉 "선수 이동"과 "공 도착"을
  같은 step에서 동시에 완성하세요.
- steps 개수는 사용자가 설명한 패스/슛 동작 수만큼 자연스럽게 정하세요
  (보통 2~6개, 최대 10개).
...
이 설명에는 패스/슛 동작이 4번 있습니다: ①윙백→미드 패스, ②미드가 A3
공간으로 리턴 패스(동시에 윙백이 그 공간으로 침투), ③윙백→스트라이커
연결, ④스트라이커 슛. 그래서 step도 정확히 4개입니다:
...
이 예시처럼 매 step마다 화살표는 하나뿐이고, 그 step이 끝나면 공은
정확히 그 화살표의 도착 구역에 가 있습니다(1단계: ball이 B2로, 2단계:
ball과 p1이 함께 A3로, 3단계: ball이 A2로, 4단계: ball이 A1로). 선수가
공 없이 미리 이동하는 별도 step은 만들지 않았다는 점에 주목하세요.`;
